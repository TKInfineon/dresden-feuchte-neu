(function () {
  ...
  var enthalpyLabel = document.createElement("label");
  enthalpyLabel.style.display = "flex";
  enthalpyLabel.style.alignItems = "center";
  enthalpyLabel.style.gap = "7px";
  enthalpyLabel.style.padding = "10px";
  enthalpyLabel.style.fontWeight = "bold";
  enthalpyLabel.innerHTML = '<input id="enthalpyToggle" type="checkbox"> Gesamtenthalpie anzeigen';
  loadBtn.parentElement.insertBefore(enthalpyLabel, loadBtn);
  var enthalpyToggle = byId("enthalpyToggle");
  ...
  function moistAirEnthalpy(temperature, humidityGPerKg) {
    var humidityRatio = humidityGPerKg / 1000;
    return 1.006 * temperature + humidityRatio * (2501 + 1.86 * temperature);
  }
  ...
  function parseResponse(data, source) {
    ...
    for (var index = 0; index < count; index++) {
      ...
      var humidity = mixingRatio(dewPoint, pressure);
      ...
      result.push({
        ...
        pressure: pressure,
        humidity: humidity,
        enthalpy: moistAirEnthalpy(temperature, humidity),
        source: source
      });
    }
  ...
  function draw() {
    ...
    var enthalpies = rows.map(function (row) { return row.enthalpy; });
    ...
    var enthalpyMin = Math.floor(Math.min.apply(null, enthalpies) - 2);
    var enthalpyMax = Math.ceil(Math.max.apply(null, enthalpies) + 2);
    if (enthalpyMin === enthalpyMax) { enthalpyMax++; }

    function enthalpyY(value) {
      return margin.top +
        (enthalpyMax - value) /
        (enthalpyMax - enthalpyMin) * plotHeight;
    }
    ...
    if (showEnthalpy) {
      drawLine(enthalpies, enthalpyY, "#6f42c1");

      context.fillStyle = "#6f42c1";
      context.font = "11px Arial";
      context.textAlign = "right";
      context.fillText(
        "Enthalpie " + enthalpyMin + " bis " + enthalpyMax + " kJ/kg tr. Luft",
  ...
  enthalpyToggle.addEventListener("change", function () {
    showEnthalpy = enthalpyToggle.checked;
    if (rows.length) { draw(); }
  });
