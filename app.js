(function () {
  ...
  function requestPast(start, end) {
  ...
  function requestFuture(start, end) {
    var forecastUrl = buildUrl(
      "https://api.open-meteo.com/v1/forecast",
      start,
      end
    );

    return fetchRows(forecastUrl, "Vorhersage");
  }
  ...
  function loadRange() {
    ...
    if (start < now) {
      var pastEnd = end < now ? end : now;
      jobs.push(requestPast(start, pastEnd));
    }

    if (end > now) {
      var futureStart = start > now ? start : now;
      jobs.push(requestFuture(futureStart, end));
    }
